function delta = find_path(alpha,eta,pheromon,energies,E_RX,E_TX,np,source_index,sink_index,neighbour_node)
    path = [];
    ns = 0;
    delta = zeros(100,100);
    while ns<np
        ...
        while finish==0
            ...
            link = possible_choices(alpha,eta,pheromon,rout);
            rout = [rout,link];
            last_node = rout(length(rout));
        ...
        cost = (cost*length(rout))/(min(energies)*mean(energies));
        for q=1:length(rout)-1
            delta(rout(q),rout(q+1))=delta(rout(q),rout(q+1)) + 1/cost;
        end
        path = [path,rout];
        ns = ns+1;
    end
